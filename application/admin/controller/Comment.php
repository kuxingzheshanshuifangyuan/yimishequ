...
class Comment extends AdminBase
{
    ...
    public function index()
    {
        ...
        $tptc = $this->commentmodel->alias('c')->join('forum f', 'f.id=c.fid')->join('user m', 'm.id=c.uid')->field('c.*,f.title,m.username,m.is_robot')->where($where)->order('c.id desc')->paginate(15,false,['query'=>$this->request->param()]);
