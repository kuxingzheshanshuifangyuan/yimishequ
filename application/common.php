...
function getbaseurl()
{
    ...
    $baseUrl = empty($baseUrl) && $baseUrl !== '\\' ? '/' : ('/' . trim($baseUrl, '/') . '/');
    return $baseUrl;
}
...
//无限级分类  主題
function get_cate($p_id=0,&$cate_one=array(),$path=0){
    /*var_dump($c_data);*/
    $path++;
    $cate=Db::table("ea_cate")
        ->where('p_id',$p_id)
        ->select();
    foreach($cate as $key=>$val){
        $val['title_ch']=str_repeat('|-- ',$path).$val['title_ch'];
        $cate_one[]=$val;
        get_cate($val['id'],$cate_one,$path);
...
function get_new_cate(){
    ...
    foreach($cates as $key=>$val){
        $forumcate=Db::name('forumcate')->where('id',$val['p_id'])->select();
        foreach ($forumcate as $k=>$v){
            $new_cates[$k]['name']=$v['name'];
            $new_cates[$k]['p_id']  =$v['id'];
        }
    }
...
//无限级分类  导航
function get_nav($nav_id=0,&$navs=array(),$path=0){
    /*dump($navs);die;*/
    $path++;
    $nav=Db::name('nav')
            ->where("nav_id",$nav_id)
   ...
   /* dump($nav);
    ...
    foreach($nav as $key=>$val){
        $val['name']=str_repeat('-- ',$path).$val['name'];
        $navs[]=$val;
        get_nav($val['id'],$navs,$path);
    }
    return $navs;
}

//获取对应 导航
function get_second_nav(){
    $navs=Db::name('nav')->select();

    foreach($navs as $key=>$val){
        $ids[]    =$val['id'];
        $nav_ids[]=$val['nav_id'];
    }
    $new_arr=array_combine($ids,$nav_ids);
    foreach($new_arr as $k=>$v){
        ...
        $new_navs[$k]['name']=$new_nav['name'];
        $new_navs[$k]['id']  =$new_nav['id'];
    }
...
//任务
function runTask($taskId,$userId,$userGroupId = 0)
{
    ...
    $task = Db::name('Task')->where(array('id' => $taskId))->find();
    ...
    if ($task) {
        ...
        if ($task['task_allow_group'] != '' && $userGroupId ) {
            // 判断下用户组
            $allowGroups = explode(',', $task['task_allow_group']);
        ...
        $class = '\\app\\common\\vendor\\task\\' . $task['task_action'];
        if (class_exists($class)) {
            $classInstance = new $class();
            return call_user_func_array([$classInstance, 'run'], [$task]);
        } else {
            return '执行器的控制器为空';
        }
    } else {
 ...
 *
 */
function getTaskStatus($taskParams,$userId,$userGroupId = ''){
    ...
    if($taskParams['task_allow_group'] != '' && $userGroupId){
        // 判断下用户组
...
function saveBase64File($images){
    $base64_image_content = $images;
    //匹配出图片的格式
    if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
        ...
        $name = md5(uniqid( microtime() . mt_rand())).".{$type}";
...
function getPic($content){
    if(preg_match_all("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\2/i", $content, $matches)) {
        ...
        foreach($matches[3] as $value){
            //排除wangEditor编辑器表情,新浪表情,网络图片
            if(strpos($value,'wangEditor') !== false || strpos($value,'sinajs.cn') !== false){
                continue;
            }else{
                $value = str_replace('http://www.1miclub.com','',$value);
                if(file_exists(ROOT_PATH.$value)){
                    $image  = \think\Image::open(ROOT_PATH.$value);
                    $width  = $image->width(); // 返回图片的宽度
                    $height = $image->height();
                    if($width < 179 || $height < 127){
                        continue;
                    }
                    return  $value;
                };
...
function autoUpGrade($userId){

    if(!$userId){
        return false;
    }

    $userInfo = Db::name('user')->field('id,point,usergrade')->where(['id'=>$userId])->find();

    if(!$userInfo){
        return false;
    }

    $choiceCount = Db::name('forum')->where(['uid'=>$userId,'choice'=>1,'open'=>1])->count();
    $hotCount    = Db::name('forum')->where(['uid'=>$userId,'view'=>['>',100],'open'=>1])->count();
    $gradeData   = Db::name('usergrade')->order('score','desc')->select();
    $friendCount = Db::name('user')->where(['tid'=>$userId,'status'=>1])->count();
    $gradeId     = 0;

    foreach ($gradeData as $key => $value){
        if($userInfo['point'] < $value['score'] || $choiceCount <  $value['choice'] || $hotCount < $value['hot']){
            continue;
        }
        if($value['is_friend'] && !$friendCount){
            continue;
        }
        $gradeId = $value['id'];
        break;
    ...
    if($userInfo['usergrade'] != $gradeId){
        $result = Db::name('user')->where(['id'=>$userId])->setField('usergrade',$gradeId);
    }
 ...
 * $moneyType 1: 积分 2:金币
 * $awardType 1: 任务 2:充值 3:
 *
 */
function addMoneyRecord($userId, $comment, $number, $moneyType = 1 ,$awardType = 0,$isReduce = 0){

    if (!$userId || !$moneyType || !$comment || !$number){
        return false;
    }
    ...
    switch ($moneyType){
        case 1:
            ...
            $result = Db::name('point_record')->insert($data);
        ...
        case 2:
            ...
            $result = Db::name('money_record')->insert($data);
            return  $result ? true : false;
    }
 ...
 * type 1: 系统消息 2:评论消息  3:回复消息
 */
function addMessageRecord( $userId, $touId, $fid = '', $comment = '', $type = 1){

    if(!$userId || !$touId || !$comment){
        return false;
    }
